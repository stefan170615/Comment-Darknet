...
layer make_batchnorm_layer(int batch, int w, int h, int c)
{
        ...
        l.inputs = w*h*c; // looks interesting, might be batch * w * h * c
        l.outputs = l.inputs;
...
{
        if(l.type == BATCHNORM) copy_cpu(l.outputs*l.batch, net.input, 1, l.output, 1); // I.output = net.input

        copy_cpu(l.outputs*l.batch, l.output, 1, l.x, 1); // I.x = net.input
        if(net.train){
