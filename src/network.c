...
load_args get_base_args(network *net)
{
        ...
        args.min = net->min_crop;                       // 608
        args.max = net->max_crop;                      // 608 * 2
        args.angle = net->angle;                          // 0
        args.aspect = net->aspect;                       // 1
        args.exposure = net->exposure;              // 1.5
        args.center = net->center;                       // 0
        args.saturation = net->saturation;          // 1.5
        args.hue = net->hue;                              // 0.1
        return args;
}
...
{
        /* *cfg = 'cfg/yolov3.cfg', *weights = 'darknet53.conv.74', clear = 0, could be */
...
{
        size_t batch_num = (*net->seen)/(net->batch*net->subdivisions); // we could just assume *net->seen = 0 right now, net->batch = 4, net->subvisions = 16. 
        return batch_num;
}
...
void forward_network(network *netp)
{
        ...
        for(i = 0; i < net.n; ++i){
                ...
                l.forward(l, net);
                net.input = l.output;
                if(l.truth) { // from the file 'cfg/yolov3.cfg' and function parse_network_cfg, we could know that, the I.truth = 0. 
                        net.truth = l.output;
                }
        }
...
float train_network(network *net, data d)
{
        ...
        int batch = net->batch;
        int n = d.X.rows / batch; // d.X.rows = 64, batch = 4, could be, so n = 16

        int i;
        float sum = 0;
        for(i = 0; i < n; ++i){
                get_next_batch(d, batch, i*batch, net->input, net->truth);
                /* to undetstand the function get_next_batch, just realize
                -- d, includes the resized images and boxes, d.X represents the images and d.y represents boxes, 
                for d.X has 64 rows, each row corresponding to one image, each image has 608*608*3 elements, 
                for d.y has 64 rows, each row corresponding to all 90 boxes related to the image, so each row has 90 * (4 + 1) elements, 
                -- batch = 4, 
                -- i*batch = i * 4, 
                -- net->input, a pointer to a float array with 608*608*3*4 elements,
                -- net->truth, a pointer to a float array with 90 *(4 + 1) * 4 elements. 
                
                so the funtion copy 4 images to the net->input and all boxes on the 4 images to the net->truth */
...
//                                                   net  im.w  im.h             0.5             0.5            0              1                         dets
void fill_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets)
{
        ...
        for(j = 0; j < net->n; ++j){
                ...
                if(l.type == REGION){
                        //                                  I    im.w    im.h      net->w  net->h    0.5       0     0.5        1       dets
                        get_region_detections(l,      w,        h,        net->w, net->h, thresh, map, hier, relative, dets);
                        dets += l.w*l.h*l.n;
                }

                if(l.type == DETECTION){
                        //                                       I     im.w     im.h        0.5    dets
