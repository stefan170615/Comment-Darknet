...
//                      the path to the loaded image         90            the pointer for store the boxes           80                      -dx/w       -dy/h      nw/w     nh/h
...
data load_data_swag(char **paths, int n, int classes, float jitter)
{
 ...
 //                                            1        the paths to the all images in the train set         the num of images in train set, could be
data load_data_detection( int n,                         char **paths,                                                         int m, 
//                                           608   608        90             80               0.3            0.1                1.5                    1.5, could be
                                          int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure )
{
        ...
        d.X.vals = calloc(d.X.rows, sizeof(float*));
        d.X.cols = h*w*3;
        /* d.X.rows, d.X.vals, d.X.cols, the variables allocated in the stack, no need to be free, 
        but the variables which d.X.vals points allocated in the dynamic memory, have to be free */
        ...
        for(i = 0; i < n; ++i) {
                ...
                float dw = jitter * orig.w; // jitter = 0.3
                ...
                //                      the path to the loaded image         90       the pointer for store the boxes           80
...
void *load_thread(void *ptr)
{
        ...
        if (a.type == OLD_CLASSIFICATION_DATA){
                ...
                *a.d = load_data_region(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
                ...
                //                                            1        the paths to the all images in the train set         the num of images in train set        
                *a.d = load_data_detection( a.n,                              a.paths,                                                          a.m,                        
                //                                           608  608         90                80         0.3       0.1          1.5               1.5      
                                                             a.w,   a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure );
        } 
...
void *load_threads(void *ptr)
{
        ...
        if (args.threads == 0) args.threads = 1; // args.threads = 64
        data *out = args.d;                                // out = &buffer
        int total = args.n;                                   // 64, the num images per batch
